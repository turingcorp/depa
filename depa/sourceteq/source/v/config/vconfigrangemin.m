...
-(instancetype)init
{
    ...
    [labeldescr setText:NSLocalizedString(@"config_rangemin_descr", nil)];
    ...
    [labeltitle setText:NSLocalizedString(@"config_rangemin_title", nil)];
...
-(void)notifiedpricemaxchanged:(NSNotification*)notification
{
...
-(void)actioncheck:(UISwitch*)check
{
    ...
    {
        self.currentprice = 0;
        [self.slider setValue:[[msettings singleton].searchmode pricemin] animated:NO];
...
-(void)savechange
{
    NSString *minstr = [NSString stringWithFormat:@"%@", @(self.currentprice)];
    [[analytics singleton] trackevent:ga_event_config_minprice action:ga_action_change label:minstr];
}
...
-(void)updaterange
{
    [self.slider setMinimumValue:[[msettings singleton].searchmode pricemin]];
